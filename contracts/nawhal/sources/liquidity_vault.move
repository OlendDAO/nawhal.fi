...
/// There can only ever be one `AdminCap` for a `LiquidityVault`
public struct VaultCap<phantom YT> has key, store {
    id: UID,
}
...
): (LiquidityVault<T, YT>, VaultCap<YT>) {
...
public fun new_with_admin_cap<T, YT>(
    _cap: &AdminCap, lp_treasury: TreasuryCap<YT>, ctx: &mut TxContext
): (LiquidityVault<T, YT>, VaultCap<YT>) {
    new<T, YT>(lp_treasury, ctx)
}

fun assert_version<T, YT>(vault: &LiquidityVault<T, YT>) {
...
public fun total_yt_supply<T, YT>(vault: &LiquidityVault<T, YT>): u64 {
...
entry fun set_tvl_cap<T, YT>(
    _cap: &VaultCap<YT>, vault: &mut LiquidityVault<T, YT>, tvl_cap: Option<u64>
) {
...
entry fun set_profit_unlock_duration_sec<T, YT>(
    _cap: &VaultCap<YT>, vault: &mut LiquidityVault<T, YT>, profit_unlock_duration_sec: u64
) {
    assert_version(vault);
...
entry fun set_performance_fee_bps<T, YT>(
    _cap: &VaultCap<YT>, vault: &mut LiquidityVault<T, YT>, performance_fee_bps: u64
) {
    assert_version(vault);
...
public fun withdraw_performance_fee<T, YT>(
    _cap: &VaultCap<YT>, vault: &mut LiquidityVault<T, YT>, amount: u64
): Balance<YT> {
    assert_version(vault);
...
public(package) fun add_strategy<T, YT>(
    _cap: &VaultCap<YT>, vault: &mut LiquidityVault<T, YT>, ctx: &mut TxContext
): VaultAccess {
    assert_version(vault);
...
entry fun set_strategy_max_borrow<T, YT>(
    _cap: &VaultCap<YT>, vault: &mut LiquidityVault<T, YT>, strategy_id: ID, max_borrow: Option<u64>
) {
    assert_version(vault);
...
entry fun set_strategy_target_alloc_weights_bps<T, YT>(
    _cap: &VaultCap<YT>, vault: &mut LiquidityVault<T, YT>, ids: vector<ID>, weights_bps: vector<u64>
) {
    assert_version(vault);
...
public fun remove_strategy<T, YT>(
    cap: &VaultCap<YT>, vault: &mut LiquidityVault<T, YT>, ticket: StrategyRemovalTicket<T, YT>,
...
entry fun migrate<T, YT>(
    _cap: &VaultCap<YT>, vault: &mut LiquidityVault<T, YT>
) {
...
public fun deposit<T, YT>(
    vault: &mut LiquidityVault<T, YT>, balance: Balance<T>, clock: &Clock
): Balance<YT> {
    assert_version(vault);
...
fun create_withdraw_ticket<T, YT>(vault: &LiquidityVault<T, YT>): WithdrawTicket<T, YT> {
...
public fun redeem_withdraw_ticket<T, YT>(
    vault: &mut LiquidityVault<T, YT>, ticket: WithdrawTicket<T, YT>
): Balance<T> {
    assert_version(vault);
...
public fun withdraw_t_amt<T, YT>(
    vault: &mut LiquidityVault<T, YT>, t_amt: u64, balance: &mut Balance<YT>, clock: &Clock
): WithdrawTicket<T, YT> {
    let total_available = total_available_balance(vault, clock);
...
public fun calc_rebalance_amounts<T, YT>(
    vault: &LiquidityVault<T, YT>, clock: &Clock
): RebalanceAmounts {
    assert!(vault.withdraw_ticket_issued == false, EWithdrawTicketIssued);
...
public(package) fun strategy_borrow<T, YT>(
    vault: &mut LiquidityVault<T, YT>, access: &VaultAccess, amount: u64
): Balance<T> {
    assert_version(vault);
    assert!(vault.withdraw_ticket_issued == false, EWithdrawTicketIssued);
...
public(package) fun strategy_hand_over_profit<T, YT>(
    vault: &mut LiquidityVault<T, YT>, access: &VaultAccess, profit: Balance<T>, clock: &Clock
) {
    assert_version(vault);
    assert!(vault.withdraw_ticket_issued == false, EWithdrawTicketIssued);
    let strategy_id = object::uid_as_inner(&access.id);
...
#[test_only]
public struct LIQUIDITY_VAULT has drop {}

#[test_only]
fun create_a_treasury(ctx: &mut TxContext): (TreasuryCap<LIQUIDITY_VAULT>, CoinMetadata<LIQUIDITY_VAULT>) {
    coin::create_currency(LIQUIDITY_VAULT{}, 6, b"ywhUSDC.e", b"", b"", option::none(), ctx)
}
...
#[test_only]
fun create_vault_for_testing(ctx: &mut TxContext): (LiquidityVault<A, LIQUIDITY_VAULT>, Balance<LIQUIDITY_VAULT>) {
...
fun test_strategy_hand_over_profit() {
    ...
    let fee_yt = balance::create_for_testing<LIQUIDITY_VAULT>(600);
...
fun test_remove_strategy() {
    ...
    let admin_cap = VaultCap<LIQUIDITY_VAULT> { id: object::new(&mut ctx) };
